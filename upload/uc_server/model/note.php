...
class notemodel {
	...
	function sendone($appid, $noteid = 0, $note = '') {
		...
		if($app['extra']['apppath'] && $this->detectescape($app['extra']['apppath'].'./api/', $apifilename) && substr(strrchr($apifilename, '.'), 1, 10) == 'php' && @include_once $app['extra']['apppath'].'./api/'.$apifilename) {
			$uc_note = new uc_note();
