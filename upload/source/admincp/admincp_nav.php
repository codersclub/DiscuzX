...
$navs = array('headernav', 'topnav', 'footernav', 'mynav', 'spacenav', 'mfindnav');
$navdata = array();
foreach($navs as $nav) {
...
EOT;
			...
			if ($ids = dimplode($_GET['delete'])) {
				C::t('common_nav')->delete_by_navtype_id(5, $_GET['delete']);
			}
		...
		$nav = C::t('common_nav')->fetch_by_id_navtype($id, 5);
		if (!$nav) {
			cpmsg('nav_not_found', '', 'error');
		}
